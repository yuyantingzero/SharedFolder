...
if($#ARGV == 1) {
  ...
  print "> Usage gen_EtoC.pl #procs \"dir\" [\"extra_args\"] < \"list\"\n";
  print "> \n";
  ...
  print ">                codegen (Put quote around a list of args, ie\n";
...
print "> Extra codegen arguments : $extra_args\n";
...
foreach $file (@files) {
  system "rm -f $directory/$file.simu.c";
}
...
for $id (1..$num_cpus) {
  ...
  if($pid) {
    ...
    foreach $file (@files) {
      ...
      unless(-e "$directory/$file.simu.c") {
	# This touch also helps prevent children from stepping on each
	# others' toes.
	system "touch $directory/$file.simu.c";

	print "> processing $directory/$file\n";
	$cmd = "codegen $extra_args -el $directory/$file -o $directory/$file.simu.c";

	if(system "$cmd") {
	  # There was an error in compilation.
	  die "> codegen failed on input file: $directory/$file\n> Using this command: $cmd\n\n";
	}
      }
    }
    exit 0;
  } else {
    die "Error using fork in gen_EtoC.pl!\n";
  }
}
