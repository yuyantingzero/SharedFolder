 ...
 * George
 *
 * $Id: pqueue.c,v 1.3 2006/09/05 05:13:15 ntclark Exp $
 *
 */
...
void PQueueInit(CtrlType *ctrl, PQueueType *queue, int maxnodes, int maxgain)
{
  ...
  if (queue->type == 1) {
    /* ntclark 9/05/06 */
    /* I'm getting segfaults, because this is too small, and I don't
     * want to figure out why.
     */
    /* queue->pgainspan = amin(PLUS_GAINSPAN, maxgain); */
    /* queue->ngainspan = amin(NEG_GAINSPAN, maxgain); */
    queue->pgainspan = PLUS_GAINSPAN;
    queue->ngainspan = NEG_GAINSPAN;
