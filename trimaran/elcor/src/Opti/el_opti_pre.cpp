...
#include "available_expr_solver.h"
#include "iterators.h"
#include "opcode_properties.h"
#include "hash_functions.h"

El_expr::El_expr()
   : opc((Opcode)0), oprnd_count(0)
{}

El_expr::El_expr(int n)
   : opc((Opcode)0), oprnd(n), oprnd_count(n)
{}

El_expr::~El_expr() {}

El_expr& El_expr::operator=(const El_expr& d1)
{
   opc = d1.opc ;
   oprnd = d1.oprnd ;
   oprnd_count = d1.oprnd_count ;
   return (*this) ;
}

bool El_expr::operator==(const El_expr& d1) const
{
   if ((opc == d1.opc) &&
       (oprnd_count == d1.oprnd_count) &&
       (oprnd == d1.oprnd)) return true ;
   else return false ;
}
...
//   cerr << "Declaring the solver" << endl << flush ;
   ...
   for(Alist_graph_dfs op_iter(graph) ; op_iter != 0 ; op_iter++) {
