 ...
 *      Added 'default' baseline parms path specifier '(* path' to parm reader.
 *      Allows users to override just a few parameters and get the rest
 *      (including other unspecified parm sections) from a different file.  
 *      Before this enhancement, every section was required to be in every 
 *      file, which was tiresome. :)
 *      Added to support new benchmark info specification framework.
 *
\*****************************************************************************/
...
extern Parm_Table_Node *L_get_parm_table_entry(Parm_Table_List *list, char *name);
...
char *current_section_name1; /* name of section that is currently being parsed. */
...
Parm_Table_List *L_create_parm_table_list (void)
{
    int i;
    Parm_Table_List *list;

    if ((list = (Parm_Table_List *) malloc (sizeof(Parm_Table_List)))==NULL)
        L_punt ("Parm_Table_List: out of memory\n");
...
void L_free_parm_table_list (Parm_Table_List *list)
{
    int i;
 ...
 */
Parm_Table_Node *L_get_parm_table_entry(Parm_Table_List *list, char *name)
{
    Parm_Table_Node *node, *next_node;
    ...
    if ((name_to_check = (char *) malloc (name_size)) == NULL)
         L_punt ("L_get_parm_table_entry: Out of memory");
...
*/
void L_add_to_parm_table_list (Parm_Table_List *list, Parm_Parse_Info *ppi,
                               char *section_name1, char *section_name2)
{
    ...
    if ((user_name = (char *) malloc (name_size)) == NULL)
         L_punt ("L_add_to_parm_table_list: Out of memory");
    ...
    for (node = list->head[hash_value]; node != NULL; node = next_node)
    {
        ...
        if (strcmp (name, node->name) == 0)
        {
           ...
           if (strcmp(ppi->name, node->parm_file_name) == 0)
               L_add_parm_already_defined(node, ppi->setting,
                                      ppi->name, ppi->line_no, section_name1,
					  section_name2);
     ...
     if ( (inp_node != NULL) && (inp_node == prev_node_location) )
        ...
        if (prev_inp_node == NULL) /* this is first element put into linked
                                      list */
           start_node_of_input = node;
 ...
 */
char *L_get_std_parm_name (char **argv, char **envp, char *env_var_name, 
			   char *default_std_parms)
{
	...
	{
	    if (argv[i+1] == NULL)
		L_punt ("-p STD_PARMS_FILE name missing");
 ...
 * See John Gyllenhaal with questions.
 */
Parm_Macro_List *L_create_external_macro_list (char **argv, char **envp)
{
	...
	*end_ptr = 0;
	 ...
	 * of that name (ie path)
         */
	if (strcmp (name_ptr, "PATH") != 0 && strcmp(value_ptr, "") != 0)
	{
	...
	if (*value_ptr != '=')
	{
	    L_punt ("Command line parse error: '=' expected after parm macro name in arg '%s'",
		   argv[i]);
	}
	...
	*end_ptr = 0;
	...
	if (*value_ptr != '=')
	{
	    L_punt ("Command line parse error: '=' expected after parm name in arg '%s'",
		   argv[i]);
	}
...
void L_warn_about_unused_macros (FILE *out, Parm_Macro_List *external_list)
{
	...
	if (node->use_count == 0)
	{
		...
		L_punt ("L_warn_about_unused_macros: algorithm error");

            if (printed == 0)
	    {
		fprintf (out, "THE FOLLOWING PARAMETERS FROM THE COMMAND LINE ARE NOT USED:\n");
		printed = 1;
            }
...
void L_show_parms_warnings(void)
{
    int printed;
    ...
    if (strcmp (L_parm_warn_file_name, "stdout") == 0)
        ...
        L_punt ("Cannot open dump output file '%s'.",
                L_parm_warn_file_name);
	...
	else 
	{
		...
		if ((dump = fopen (L_parm_dump_file_name, "w")) == NULL)
		    L_punt ("Cannot open dump output file '%s' for writing.",
			    L_parm_dump_file_name);
	    }
	    else
	    {
		if ((dump = fopen (L_parm_dump_file_name, "a")) == NULL)
		    L_punt ("Cannot open dump output file '%s' for appending.",
			    L_parm_dump_file_name);
	    }
	}
	...
	else
	{
	   ...
	   if (inp_node->used == USED)
              fprintf(dump, "    %s = %s;\n", inp_node->user_name, inp_node->value);
    ...
    if (L_warn_parm_not_used)
    {
		...
		{
		    fprintf (out, "    '%s' in section '%s' aka '%s' ('%s' line %d).\n",
			     node->name, node->section_name1,
			     node->section_name2,
			     node->parm_file_name, node->parm_line_no);
		}
    ...
    if (L_warn_parm_not_defined)
    {
        for (node = warn_list->head; node != NULL; node = node->next)
            if (node->flag == NO_PARM_DEF)
            {
                ...
                {
                     printed = 1;
                     fprintf(out, "\nTHE FOLLOWING PARAMETERS WERE NOT SET IN THE PARAMETER FILE:\n");
                }
		if (node->section_name2 == NULL)
		{
    ...
    if (L_warn_parm_defined_twice)
    {
        for (node = warn_list->head; node != NULL; node = node->next)
            if (node->flag == MULT_PARM_DEF)
            {
                ...
                {
                     printed = 1;
                     fprintf(out, "\nTHE FOLLOWING PARAMETERS WERE SET TWICE IN THE SAME PARAMETER FILE:\n");
                }
...
Parm_Macro_List *L_create_parm_macro_list (void)
{
    Parm_Macro_List *list;

    if ((list = (Parm_Macro_List *) malloc (sizeof(Parm_Macro_List))) == NULL)
	L_punt ("Parm_Macro_List: out of memory\n");
...
void L_free_parm_macro_list (Parm_Macro_List *list)
{
...
int look_for_forced_param(char *name, Parm_Macro_List *list)
{
   Parm_Macro_Node *node;
   Parm_Parse_Info ppi;
            ...
            node = node->next)
       {
           ...
           if ( !strcmp(node->name, name) )
           {
              ...
              if ((ppi.name = (char *)malloc(10)) == NULL)
                 L_punt("look_for_forced_param: out of memory");
...
char *L_add_forced_parms(Parm_Parse_Info *ppi, Parm_Table_List *list)
{
   Parm_Macro_Node *node;
	   ...
	   if ((parm_node = L_get_parm_table_entry(list, node->name)) != 0)
	   {
	       ...
	       /* Change value in the hash list */
               ...
               if ((parm_node->value = (char *) malloc(value_size)) == NULL)
                  L_punt("L_add_forced_parms: Out of memory");
...
char *L_get_parm_macro_value(Parm_Parse_Info *ppi, char *name)
{
    Parm_Macro_Node *node;
    int i;
			 ...
			 int from)
{
    ...
    if (list == NULL)
	L_punt ("L_define_parm_macro: null list");
    ...
    *ptr = 0;
	...
	L_punt ("L_define_parm_macro: out of memory");
...
void L_expand_parm_macros (Parm_Parse_Info *ppi, char *line, char *buf, 
			   int max)
{
    int i, j, value_size;
	...
	if (strlen(forced_value) >= (max - 1))
	    L_punt ("%s line %i: expanded line too long (max len %i)",
	...
	if (*line == '\\')
	{
	    ...
	    if (line[1] == 0)
		L_punt ("%s line %i: cannot end line with a '\\'.\n",
	...
	else
	{
		...
		{
		    name_buf[j] = 0;
		    L_punt ("%s line %i: macro name contains space after '%s'",
		...
		if (j >= (int) sizeof (name_buf))
		{
		    ...
		    L_punt ("%s line %i: macro name too long '%s...'",
			    ppi->name, ppi->line_no, name_buf);
		}
	    ...
	    if (*line != '$')
		L_punt ("%s line %i: ill-formed macro '%s'",
			    ppi->name, ppi->line_no, name_buf);

	    /* Get macro's value */
	    if ((value = L_get_parm_macro_value(ppi, name_buf)) == NULL)
		L_punt ("%s line %i: Undefined macro '%s'",
			    ppi->name, ppi->line_no, name_buf);
 ...
 */
void L_set_parm_s (char **parm, char *new_string)
{
    int size;
    char *new_str;
	...
	L_punt ("set_parm_s: Out of memory");
				 ...
				 char *section_name2)
{
   ...
   if (((node = (Parm_Warn_Node *) malloc (node_size)) == NULL) ||
       ...
       ((node->parm_file_name = (char *) malloc (file_name_size)) == NULL) ||
       ((node->old_parm_file_name = (char *) malloc (old_file_name_size)) == NULL))
           L_punt ("L_add_parm_defined_already: Out of memory");
   ...
   if (section_name2 != NULL)
   {
       ...
       if ((node->section_name2 = (char *)malloc (section_name_size2)) == NULL)
           L_punt ("L_add_parm_defined_already: Out of memory");
 ...
 * out after similar warnings that have already been marked.
 */
void L_add_parm_not_used (Parm_Table_List *parm_list,
                          char *section_name1, char *section_name2)
{
        ...
        for (parm_node = parm_list->head[i]; parm_node != NULL;
            ...
            if (parm_node->used == NOT_USED)
            {
                ...
                if (((node = (Parm_Warn_Node *) malloc (node_size)) == NULL) ||
                    ...
                    ((node->section_name1 = (char *) malloc (section_name_size1)) == NULL) ||
                    ((node->parm_file_name = (char *) malloc (file_name_size)) == NULL))
                    L_punt ("L_add_parm_not_used: Out of memory");
 ...
 */
void L_add_parm_not_defined (char *name, char *section_name1, 
			     char *section_name2)
{
    ...
    if (((node = (Parm_Warn_Node *) malloc (node_size)) == NULL) ||
        ((node->name = (char *) malloc (name_size)) == NULL) ||
        ((node->section_name1 = (char *) malloc (section_name_size1)) == NULL))
        L_punt ("L_add_parm_not_defined: Out of memory");
...
/* Reads in an int parameter.  Nothing else may be on the line. 
  */
void L_read_parm_i (Parm_Parse_Info *ppi_in, char *name, int *ptr)
{
    Parm_Table_List *list;
 ...
 */
void L_read_parm_b (Parm_Parse_Info *ppi_in, char *name, int *ptr)
{
    Parm_Table_List *list;
    Parm_Table_Node *node;    
    int dev_parm;
	...
	*ptr = 1;
	...
	L_pmatch (node->value, "1") || 
	L_pmatch (node->value, "on"))
	*ptr = 1;
    
    else if (L_pmatch (node->value, "no") ||
	     L_pmatch (node->value, "n") ||
	     L_pmatch (node->value, "0") ||
	     L_pmatch (node->value, "off") )
	*ptr = 0;

    else
	L_punt ("%s line %i: parm '%s' expects a binary setting, not '%s'",
		node->parm_file_name, node->parm_line_no, node->name, node->value);
 ...
 */
void L_read_parm_s (Parm_Parse_Info *ppi_in, char *name, char **ptr)
{
    Parm_Table_List *list;
    Parm_Table_Node *node;
    int dev_parm;
 ...
 */
void L_read_parm_f (Parm_Parse_Info *ppi_in, char *name, float *ptr)
{
    Parm_Table_List *list;
    Parm_Table_Node *node;
    char *end_ptr;
    ...
    *ptr = strtod (node->value, &end_ptr);
    ...
    if (*end_ptr != 0)
    {
	L_punt ("%s line %i parameter %s: invalid float '%s'",
		node->parm_file_name, node->parm_line_no, node->name, node->value);
    }
 ...
 */
void L_read_parm_lf (Parm_Parse_Info *ppi_in, char *name, double *ptr)
{
    Parm_Table_List *list;
    Parm_Table_Node *node;
    char *end_ptr;
    ...
    *ptr = strtod (node->value, &end_ptr);
    ...
    if (*end_ptr != 0)
    {
	L_punt ("%s line %i parameter %s: invalid double '%s'",
		node->parm_file_name, node->parm_line_no, node->name, node->value);
    }
	...
	*parm = 0;
	...
	if (ppi->parm_buf[0] == '$')
	{
		...
		if (isspace(ppi->parm_buf[i]))
		    L_punt ("Macro name '%s' cannot have whitespace in it\n",
			    ppi->parm_buf);
	    }
	    
	    if ((ppi->parm_buf[0] != '$') || (ppi->parm_buf[i-1] != '$') ||
                (i < 3) || (count != 2))
		L_punt ("Badly formed macro name '%s'\n", ppi->parm_buf);
...
int L_parm_calc_complete_expr (Parm_Parse_Info *ppi)
{
    ...
    if (ppi->calc_ptr[0] != 0)
    {
	L_punt ("%s line %i: Expr formulation error.  "
		"Unable to process '%s'.", 
...
int L_parm_calc_factor(Parm_Parse_Info *ppi)
{
	...
	if (ppi->token == TNUM)
	    ...
	    L_punt ("%s line %i: parse_error, expected number after '-'.",
		    ppi->name, ppi->line_no);
      case TNUM:
	return (ppi->value);
	...
	L_get_next_token (ppi);
	if (ppi->token != TRPARN)
	    L_punt ("%s line %i: ')' expected.", ppi->name, ppi->line_no);
	return (result);

      case TRPARN:
	    L_punt ("%s line %i: unexpected ')' before '%s'.", 
		    ppi->name, ppi->line_no, ppi->calc_ptr);
	
      default:
	    L_punt ("%s line %i: Expression formulation error before '%s'.", 
		    ppi->name,  ppi->line_no, ppi->calc_ptr);
...
int L_peek_next_token (Parm_Parse_Info *ppi)
{
	...
	switch (*ptr)
	{
	  ...
	  case ')':
	    ...
	    break;
	  default:
	    L_punt ("%s line %i: Parse error at '%c' in '%s'.",
...
void L_read_parm_section_defaults (Parm_Parse_Info *ppi, 
                                   ...
                                   char *defaults_file_name)
{
	...
	if (psi->name2 == NULL)
	{
	    L_punt ("Recursed too many times reading parameter defaults "
		    " for '%s'.",  psi->name1);
	}
	else
	{
	...
	if (psi->name2 == NULL)
	{
	    L_punt ("Unable to open parameter defaults file '%s'.\n"
...
void L_read_parm_sections(Parm_Parse_Info *ppi, Parm_Section_Info *psi)
{
	...
	if (L_pmatch(ppi->parm_buf, "(*"))
	{
		...
		if ((ppi->setting[0] == 0))
		{
		    L_punt("A 'defaults_path' expected after '(*' on line %i of '%s'.",
	    ...
	    {
		L_punt("Expected 'end)' after '(*' on line %i of '%s'.",
		       ppi->line_no, ppi->name);
	    }
	    ...
	    {
		L_punt("Expected 'end)' not '%s' after '(*'\n"
		       "on line %i of '%s'.",
	...
	else if ((psi->name2 != NULL) && L_pmatch (ppi->parm_buf, psi->name2))
	{
		...
		/* Load the parameters into the hash table */
		while (L_get_next_parm (ppi) && !L_pmatch (ppi->parm_buf, "end)"))
	...
	else
	{
	    ...
	    L_punt ("Cannot continue: missing parameter file section in %s.",
		    ppi->name);
	}
    }
 ...
 */
void _L_load_parameters (char *parm_file_name,
			 ...
			 void (*read_func)(Parm_Parse_Info *ppi))
{
	...
	fprintf (stderr, "Unable to open parameter file '%s'.\n ",
		 parm_file_name);
	L_punt ("Check setting of environment variable STD_PARMS_FILE.");
    }

    /* initailize the warn list pointer to null.
     * the pointer must be initialized for the warnings to be added correctly.
     */
    if ((warn_list = (Parm_Warn_List *) malloc (sizeof(Parm_Warn_List))) == NULL)
        L_punt ("out of memory");
    warn_list->head = NULL;
	...
	if (psi->name2 != NULL)
	{
	    L_punt ("_L_load_parameters: Neither section %s or %s used!",
		    psi->name1, psi->name2);
	}
	else
	{
	    L_punt ("_L_load_parameters: Section not used!",
		    psi->name1);
	}
    }
...
void L_load_parameters (char *parm_file_name,
                        Parm_Macro_List *command_line_macro_list,
...
void L_load_parameters_aliased (char *parm_file_name,
				Parm_Macro_List *command_line_macro_list,
				...
				void (*read_func)(Parm_Parse_Info *ppi))
{
    ...
    _L_load_parameters (parm_file_name, command_line_macro_list,
			new_section_name, old_section_name, 
			read_func);
}
