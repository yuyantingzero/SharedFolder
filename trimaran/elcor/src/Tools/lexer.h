...
#ifndef _LEX_INCLUDED_
#define _LEX_INCLUDED_
...
class Lexer {
  public:
    ...
    Lexer &get(double &);
    Lexer &get(char *);
    Lexer &get(Token &); // useful when the type of next token is unknown.;
    Lexer &check(const char*);
    Lexer &check(const char);
  ...
  private:
    istream *in;	// This used to be _IO_ostream_withassign
