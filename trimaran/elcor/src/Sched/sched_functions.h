...
#ifndef _SCHED_FUNCTIONS_H
#define _SCHED_FUNCTIONS_H
...
void calculate_earliest_times(Compound_region* region, 
     const Relax_dep_constr* relax_dep = NULL, 
     const Op_filter* ofilter = ALL_OPS,
     const Edge_filter* einfilter = ALL_EDGES);
...
void calculate_latest_times_from_exit_op(Compound_region* region,
		Op *exit_op, 
...
Op* get_next_ready_node(List<Op*>& op_list, const Op_filter* filter = ALL_OPS,
					   Motion_type type = MT_NONE);
...
double count_cycles_for_schedule (Compound_region *reg, double *branch_count,
				  double *dc_stall_cycle_count);
