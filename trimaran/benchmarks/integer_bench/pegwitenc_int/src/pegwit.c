...
int main( unsigned argc, char * argv[] )
{
  ...
  if (operation == 'e') 
    stdinfp = fopen("encryption_junk", "r");
  else if (operation == 'd')
    stdinfp = fopen("my.sec", "r");
  else
    stdinfp = stdin;

  if(!stdinfp)
  {
    fprintf(stderr, "encryption_junk file is missing");
    exit(1);
  }
  ...
  if(!filter)
  {
    ...
    f_sec = 0;
    if('e' == operation || 'v' == operation) f_sec = stdinfp;
    if ( argc > arg_ix )
    {
